...
from premortem_ai.config.settings import settings
...
def _get_openai_client():
    ...
    if settings.ENVIRONMENT == "local":
        raise RuntimeError(
            "OpenAI client requested in local environment. "
            "This is a bug — local must use MockLLMClient."
        )

    if _client is None:
        from openai import OpenAI  # 👈 import happens ONLY here
